...
export function missingMetaScopesMessage(
  ...
  return (
    ...
    `الحل خطوة واحدة: افصل ${name} من صفحة التكاملات ثم اربطه من جديد — نافذة الربط لدينا ` +
    `تطلب الآن صلاحيات إدارة الصفحة والنشر، واختر الصفحة التي تريد النشر عليها ووافق على كل الأذونات. ` +
    `تأكد أنك مسؤول (Admin) على الصفحة قبل الربط.`
  );
