...
export function missingMetaScopesMessage(
  ...
  return (
    `${name} أعطى صلاحية «عرض الصفحات» فقط ولم يعطِ صلاحية النشر (${missing.join("، ")}). ` +
    `هذا قرار من ميتا وليس عطلاً في الحساب: نافذة الربط الافتراضية لا تطلب صلاحيات النشر أصلاً. ` +
    `الحل النهائي بثلاث خطوات: (١) أنشئ تطبيقاً في لوحة مطوري ميتا واطلب له Advanced Access للصلاحيتين ` +
    `pages_manage_posts و pages_read_engagement. (٢) أضِف مفتاحي التطبيق (App ID/Secret) كـ OAuth Client داخل ` +
    `مشروعكم في Pipedream وانسخ معرّف الـ OAuth Client. (٣) الصقه في الإعدادات ← المفاتيح باسم ` +
    `PIPEDREAM_OAUTH_APP_${provider === "facebook" ? "FACEBOOK" : "INSTAGRAM"} ثم افصل ${name} وأعد ربطه. ` +
    `بعدها يعمل النشر مباشرة بلا أي تعديل آخر.`
  );
}
